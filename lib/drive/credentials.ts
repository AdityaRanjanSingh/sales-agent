 ...
 */
export async function getDriveAccessTokenFunctionForUser(userId: string): Promise<() => Promise<string>> {
  if (!userId) {
    console.error('[Drive Credentials] Error: User ID not provided');
  ...
  return async () => {
    ...
    try {
      ...
      if (!googleAccount) {
        ...
        console.error('[Drive Credentials] Error: No verified Google account found for user:', userId, {
        ...
        throw new Error(
          "Google Drive not connected. Please sign in with Google to enable Drive integration."
        );
      }
      ...
      if (!token || !token.data || token.data.length === 0) {
        console.error('[Drive Credentials] Error: Failed to retrieve OAuth token from Clerk', {
          hasToken: !!token,
          hasData: !!token?.data,
          dataLength: token?.data?.length || 0
        });
        throw new Error(
          "Failed to get Google Drive OAuth token. Please reconnect your Google account."
        );
      }
      ...
      if (!accessToken) {
        console.error('[Drive Credentials] Error: Access token is empty or undefined');
      ...
      console.error(`[Drive Credentials] Error retrieving token (${duration}ms):`, {
...
/**
 * Gets an access token function that can be used by Google Drive tools
 * Retrieves the Google OAuth token directly from Clerk's OAuth provider
 */
export async function getDriveAccessTokenFunction(): Promise<() => Promise<string>> {
  const { userId } = await auth();

  if (!userId) {
    console.error('[Drive Credentials] Error: User not authenticated');
    throw new Error("User not authenticated");
  }

  return getDriveAccessTokenFunctionForUser(userId);
}
